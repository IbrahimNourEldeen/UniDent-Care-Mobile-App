    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      <ScrollView className="flex-1 px-5 pt-4">
        ...
        {mockCases.map((item) => (
          ...
          >
            <View
              className={`w-12 h-12 rounded-xl items-center justify-center ${item.status === "Pending" ? "bg-orange-100" : "bg-green-100"}`}
            >
              <FontAwesome5
                name="user-alt"
                size={18}
            ...
            <View className="items-end">
              ...
              <Text
                className={`text-[10px] font-black uppercase tracking-tighter ${item.status === "Pending" ? "text-orange-500" : "text-green-500"}`}
              >
                {item.status}
              </Text>
            </View>
