...
import { CalendarPlus, Play, CalendarClock, Trash2, RefreshCw, Square } from 'lucide-react-native';
...
interface Props {
  ...
  // InProgress session (for End Session button)
  inProgressSession?: any | null;
  // Start Now modal
  ...
  // End session (navigate to active session)
...
export default function ScheduleSessionSection({
  ...
  // Mirror web logic: session is expired if date < today (day-level comparison)
  const isExpired =
    scheduledSession?.status?.toString().toLowerCase() === 'expired' ||
    scheduledSession?.status?.toString() === '3' ||
    (() => {
  ...
  return (
    <View className="space-y-3">
      ...
      {/* ── InProgress Session: End Session card ──────────────────────────── */}
      {inProgressSession && (
        <View
        ...
        >
          <View className="flex-row items-center gap-1.5">
            <Play size={13} color={isDark ? '#fbbf24' : '#d97706'} />
            <Text
              className={`text-xs font-semibold uppercase tracking-wide ${
                isDark ? 'text-amber-400' : 'text-amber-600'
              }`}
            >
              {locale === 'ar' ? 'جلسة جارية' : 'Session In Progress'}
            </Text>
          </View>

          <View className="flex-row items-center gap-2">
            <CalendarClock size={14} color={isDark ? '#fbbf24' : '#d97706'} />
            <Text
              className={`text-sm font-medium ${
                isDark ? 'text-slate-200' : 'text-slate-700'
              }`}
            >
              {formatSessionTime(inProgressSession.scheduledAt)}
            </Text>
          </View>

          <TouchableOpacity
            onPress={onGoToActiveSession}
            activeOpacity={0.8}
            className={`flex-row items-center justify-center gap-2 rounded-xl py-2.5 shadow-sm ${
      ...
      {hasScheduledSession && !isExpired && (
        ...
        >
          ...
          {/* Cancel + Reschedule — matches web exactly */}
          <View className="flex-row items-center gap-2 pt-1">
            <TouchableOpacity
              ...
              className={`flex-1 flex-row items-center justify-center gap-1.5 rounded-xl border py-2.5 ${
                isDark
                  ? 'bg-rose-900/10 border-rose-800/60'
            ...
            {/* Reschedule — matches web (NOT Start Now) */}
            <TouchableOpacity
              disabled={sessionLoading}
              onPress={() => onToggleForm(true)}
              activeOpacity={0.8}
              className={`flex-1 flex-row items-center justify-center gap-1.5 rounded-xl shadow-sm py-2.5 ${
      ...
      {hasScheduledSession && isExpired && (
        <View
          className={`rounded-2xl p-4 border space-y-3 ${
            isDark
              ? 'bg-rose-900/10 border-rose-800/50'
        ...
        >
          ...
          <View className="flex-row items-center gap-2">
            ...
            <Text
              className={`text-sm font-medium line-through opacity-70 ${
                isDark ? 'text-slate-200' : 'text-slate-700'
              }`}
            >
              {formatSessionTime(scheduledSession.scheduledAt)}
      ...
      {/* ── No session: Schedule New ─────────────────────────────────────── */}
      {!hasScheduledSession && !inProgressSession && (
        <TouchableOpacity
          disabled={sessionLoading}
          onPress={() => onToggleForm(true)}
          activeOpacity={0.8}
          className={`flex-row items-center justify-center gap-2 py-3.5 rounded-xl shadow-sm ${
            isDark ? 'bg-indigo-600' : 'bg-indigo-600'
          }`}
        >
          {sessionLoading ? (
      ...
      {/* Booking Dialog — passes hasScheduledSession & onToggleStartNowModal so "Start Now" pill shows in Reschedule mode */}
